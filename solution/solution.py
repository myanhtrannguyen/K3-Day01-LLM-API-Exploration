...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        import tiktoken
        # Sẽ ném ra KeyError nếu model lạ không có trong từ điển của tiktoken
        enc = tiktoken.encoding_for_model(model)
        return len(enc.encode(text))
    except (KeyError, Exception):
        # Fallback ước lượng dự phòng nếu lỗi (offline, thư viện lỗi, model lạ)
...
def streaming_chatbot() -> None:
    ...
    # Dùng fallback "dummy" để đảm bảo an toàn nếu môi trường test không nạp .env
    ...
    while True:
        try:
            # Bỏ chuỗi "You: " trong input() để không làm hỏng bài test tự động
            user_msg = input()
        ...
        reply = ""
        for chunk in stream:
            # Lấy token an toàn, fallback về chuỗi rỗng
            delta = chunk.choices[0].delta.content or ""
            reply += delta
            # Chỉ in duy nhất token, không in chữ "Assistant: "
            print(delta, end="", flush=True)
            
        print()  # Xuống dòng sau khi in xong toàn bộ reply
        
        history.append({"role": "assistant", "content": reply})
        history = history[-6:]  # Giữ tối đa 6 tin nhắn (3 lượt)
    ...
    while True:
        try:
            ...
            if attempt >= max_retries:
                raise e
            # Delay tăng theo cấp số nhân: 0.1s, 0.2s, 0.4s...
...
def run_assistant(
    ...
    while True:
        ...
        try:
            # Ghi đè input để test tự động hoặc để tương tác thật
            user_msg = get_input("You: ") if get_input is input else get_input()
        except EOFError:
            break
        ...
        # Chuẩn bị messages gồm system, history và tin nhắn mới nhất
        messages = [{"role": "system", "content": persona}] + history + [{"role": "user", "content": user_msg}]
        
        # Bọc lời gọi trong lambda để truyền vào retry_with_backoff
        def make_api_call():
        ...
        # Tính toán chi phí (Dùng str(messages) để bao gồm cả lịch sử vào input cost)
        cost_stats = estimate_cost(str(messages), reply, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = f"{'Prompt':<30} | {'GPT-4o Response':<30} | {'Mini Response':<30} | {'Lat 4o':<10} | {'Lat Mini':<10}"
    separator = "-" * len(header)
    lines = [header, separator]
    
    for row in results:
        p = (row['prompt'][:27] + "...") if len(row['prompt']) > 30 else row['prompt']
        g = (row['gpt4o_response'][:27] + "...") if len(row['gpt4o_response']) > 30 else row['gpt4o_response']
        m = (row['mini_response'][:27] + "...") if len(row['mini_response']) > 30 else row['mini_response']
        
        l_gpt = f"{row['gpt4o_latency']:.2f}s"
        l_mini = f"{row['mini_latency']:.2f}s"
        
        lines.append(f"{p:<30} | {g:<30} | {m:<30} | {l_gpt:<10} | {l_mini:<10}")
...
if __name__ == "__main__":
    ...
    #         print(f"{key}: {value}")
    text = (
        "Trí tuệ nhân tạo (AI) đang phát triển với tốc độ chóng mặt, thay đổi cách "
        "chúng ta làm việc và sinh hoạt mỗi ngày. Trong tương lai gần, AI không chỉ "
        "giới hạn ở việc tự động hóa các tác vụ lặp đi lặp lại mà còn tham gia vào "
        "quá trình sáng tạo nghệ thuật, chẩn đoán y khoa và lập kế hoạch chiến lược. "
        "Mặc dù mang lại vô vàn lợi ích to lớn, sự phát triển của công nghệ này cũng "
        "đặt ra nhiều thách thức về đạo đức, quyền riêng tư và vấn đề việc làm. Do đó, "
        "con người cần phải học cách thích nghi và làm chủ công nghệ, đảm bảo rằng AI "
    ...
    # 2. Ước lượng theo công thức Part 1 (1 token ≈ 0.75 từ)
    estimated_tokens = word_count / 0.75
    
    # 3. Đếm token thực tế bằng tiktoken
    try:
        actual_tokens = count_tokens(text, model="gpt-4o")
        
        # 4. Tính độ chênh lệch
        diff_percent = ((actual_tokens - estimated_tokens) / estimated_tokens) * 100
        
        print("=== TEST TIKTOKEN VS ĐẾM TỪ (TIẾNG VIỆT) ===")
        print(f"Số từ thực tế: {word_count} từ")
        print(f"Số token ước lượng (từ / 0.75): {estimated_tokens:.1f} tokens")
        print(f"Số token thực tế (tiktoken): {actual_tokens} tokens")
        print(f"Độ chênh lệch: Thực tế cao hơn ước lượng {diff_percent:.1f}%")
    except Exception as e:
        print(f"Lỗi: {e}")
